        ...
        <div class="block">
            <div class="block-header bg-gray-lighter">
                ...
                <h3 class="block-title">
                    {!! Form::button('<i class="fa fa-plus"></i> Adicionar', ['class'=>'btn btn-primary', 'onclick'=>'window.open(\''.route('appsAdd').'\', \'_self\');']) !!}
                    {!! Form::button('<i class="fa fa-list"></i> Ordenar', ['class'=>'btn btn-warning push-10-l', 'onclick'=>'window.open(\''.route('appsOrder').'\', \'_self\');']) !!}
                </h3>
            </div>
            <div class="block-content">
                ...
                <table class="table table-bordered table-striped js-dataTable-full">
                    ...
                    @foreach($apps as $app)
                        <tr>
                            <td>{{ $app->sortorder }}</td>
                            <td class="font-w600">{{ $app->title }}</td>
                            <td class="text-center">
                                {!! Form::button('<i class="fa fa-pencil"></i>', ['title'=>'Editar', 'data-toggle'=>'tooltip', 'class'=>'btn btn-xs btn-primary',
                                'onclick'=>'window.open(\''.route('appsEdit', $app->appsId).'\', \'_self\')']) !!}

                                {!! Form::open([
                                    'id' => 'textDelete'.$app->appsId,
                                    'method' => 'delete',
