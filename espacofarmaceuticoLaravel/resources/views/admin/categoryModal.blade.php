...
$(document).ready(function(){
    ...
    $('#form_<?=$modalName;?>').submit(function(e){
        ...
        if(title.length == 0){
            ...
            bootbox.alert("Informe algum texto");
            ...
            $.ajax({
                ...
                if (data.error == 1) {
                    bootbox.alert("Texto já existente");
                    $('input#title<?=$modalName;?>').val('');
                }
                if (data.error == 2) {
                    bootbox.alert("Ocorreu um erro ao salvar, por favor tente novamente!");
                }
                if (data.error == 3) {
                    bootbox.alert("Erro!<br />Informe todos os campos!");
                }
        ...
        if(key == 13){
            if(inputRequired == 'required' && inputValue == ''){
                ...
                $.ajax({
                    ...
                    if (data.error == 1) {
                        bootbox.alert("Texto já existente", function (){
                            $('input#categoryTitle'+id).val('');
                            $('input#categoryTitle'+id).focus();
                        });
                    }
                    if (data.error == 2) {
                        bootbox.alert("Ocorreu um erro ao salvar, por favor tente novamente!", function (){
                            $('input#categoryTitle'+id).focus();
                        });
                    }
                    if (data.error == 3) {
    ...
    $('.btn-delete').click(function(e) {
        e.preventDefault();
        var id = $(this).attr('data-id');
        bootbox.confirm('Tem certeza que deseja deletar?<br>Isso será um ato irreversível e todos os ítens com relação à ele ficarão sem referência!', function(result){
            if (result == true) {
                ...
                $.ajax({
                    ...
                    if(data.error == 1) {
                        bootbox.alert("Erro ao deletar<br />Por favor, tente novamente!");
                    }
                    if (data.error == 2) {
                        bootbox.alert("Ocorreu um erro ao excluir");
                    }
                    if (data.error == 3) {
                        bootbox.alert("Ocorreu um erro ao excluir");
                    }
    ...
    //$('[data-dismiss="modal"]').click(function(){
        ...
        $.ajax({
            ...
            var categories = '<option value="">Escolher...</option>';
    ...
    <div class="modal-content">
        ...
        <div class="block block-themed block-transparent">
            ...
            <div class="block-content">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-8 col-md-8 col-sm-10 col-xs-10">
                            <div class="form-input">
                                {!! Form::text('title'.$modalName, '', ['id' => 'title'.$modalName, 'placeholder' => 'Título', 'maxlength' => '45', 'class' => 'form-control']) !!}
                            </div>
                        </div>
                        <div class="col-lg-4 col-md-4 col-sm-2 col-xs-2" style="text-align:left;">
                            {!! Form::submit('Gravar', ['id' => 'btn-add', 'class' => 'btn btn-primary']) !!}
                        </div>
                    </div>
                {!! Form::close() !!}
                ...
                <table class="responsive table table-striped table-bordered dataTable categoryList">
                    <thead>
                        <tr role="row">
                            ...
                            <th>Título</th>
                            <th class="sorting_none" style="width:80px; text-align:center;">Ações</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php $bg2 = 0; ?>
                        ...
                        <tr class="<?=$ex2;?>" id="{{ $category->id }}">
                            ...
                            <td align="center">
                                {!! Form::button('<i class="fa fa-pencil"></i>', ['title'=>'Editar', 'data-id' => $category->id, 'class'=>'btn btn-xs btn-primary btn-edit']) !!}

                                {!! Form::button('<i class="fa fa-trash"></i>', ['title'=>'Excluir', 'data-id' => $category->id, 'class'=>'btn btn-xs btn-danger btn-delete']) !!}
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
                ...
                @else
                <center>Nenhum(a) {{ $modalTitle }}</center>
                @endif
            </div>
        </div>
        <!-- END Apps Block -->
