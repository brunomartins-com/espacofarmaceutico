        ...
        <div class="block">
            <div class="block-header bg-gray-lighter">
                ...
                <h3 class="block-title">
                    {!! Form::button('<i class="fa fa-plus"></i> Adicionar', ['class'=>'btn btn-primary', 'onclick'=>'window.open(\''.route('legislationAdd').'\', \'_self\');']) !!}
                    {!! Form::button('<i class="fa fa-dedent"></i> Texto', ['class'=>'btn btn-warning push-10-l', 'onclick'=>'window.open(\''.route('legislationText').'\', \'_self\');']) !!}
                </h3>
            </div>
            <div class="block-content">
                ...
                <table class="table table-bordered table-striped js-dataTable-full">
                    ...
                    <tbody>
                    @foreach($legislation as $text)
                        <tr>
                            <td>{{ $text->legislationId }}</td>
                            <td>{{ $text->category->legislationCategoriesName }}</td>
                            ...
                            <td class="text-center">
                                ...
                                'onclick'=>'window.open(\''.route('legislationEdit', $text->legislationId).'\', \'_self\')']) !!}

                                {!! Form::open([
                                    'id' => 'textDelete'.$text->legislationId,
                                    'method' => 'delete',
                                    'enctype' => 'multipart/form-data',
                                    'url' => ''
                                    ])
                                !!}
                                {!! Form::hidden('legislationId', $text->legislationId) !!}
                                {!! Form::button('<i class="fa fa-trash"></i>', ['title'=>'Excluir', 'data-toggle'=>'tooltip', 'class'=>'btn btn-xs btn-danger btn-delete',
                                'data-url'=>route('legislationDelete'), 'data-form'=>true, 'data-id-form'=>'textDelete'.$text->legislationId]) !!}
                                {!! Form::close() !!}
                            </td>
                        </tr>
                    @endforeach
