...
@section('title', 'Ordenar (Legislação) | ')
                ...
                <div class="block-content block-content-full">
                    ...
                    <div class="row js-draggable-items">
                        <div class="col-sm-12 draggable-column">
                            ...
                            !!}
                            {!! Form::hidden('databaseTable', 'legislation') !!}
                            {!! Form::hidden('primaryKey', 'legislationId') !!}
                            {!! Form::hidden('sortorder', '') !!}
                            @foreach($items as $item)
                                    <!-- Block -->
                            <div class="block draggable-item" title="{{ $item->legislationId }}">
                                <div class="block-header">
                                    ...
                                    <h3 class="block-title">{{ $item->title }} / Categoria: {{ $item->category->legislationCategoriesName }}</h3>
                                </div>
                            </div>
                            <!-- END Block -->
                            @endforeach
                            <div class="form-group">
                                <div class="col-xs-12 push-30-t">
                                    {!! Form::button('Concluído', ['class'=>'btn btn-primary btn-back pull-left', 'data-url'=>route('legislation')]) !!}
                                </div>
                            </div>
                            {!! Form::close() !!}
                        </div>
