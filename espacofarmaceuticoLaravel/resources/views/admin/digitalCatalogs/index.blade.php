...
@section('title', 'Catálogos Digitais | ')
        ...
        <div class="block">
            <div class="block-header bg-gray-lighter">
                ...
                <h3 class="block-title">
                    {!! Form::button('<i class="fa fa-plus"></i> Adicionar', ['class'=>'btn btn-primary', 'onclick'=>'window.open(\''.route('digitalCatalogsAdd').'\', \'_self\');']) !!}
                    {!! Form::button('<i class="fa fa-list"></i> Ordenar', ['class'=>'btn btn-warning push-10-l', 'onclick'=>'window.open(\''.route('digitalCatalogsOrder').'\', \'_self\');']) !!}
                </h3>
            </div>
            <div class="block-content">
                ...
                <table class="table table-bordered table-striped js-dataTable-full">
                    ...
                    <tbody>
                    @foreach($digitalCatalogs as $text)
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                'onclick'=>'window.open(\''.route('digitalCatalogsEdit', $text->digitalCatalogsId).'\', \'_self\')']) !!}

                                {!! Form::open([
                                    'id' => 'textDelete'.$text->digitalCatalogsId,
                                    'method' => 'delete',
                                    'enctype' => 'multipart/form-data',
                                    'url' => ''
                                    ])
                                !!}
                                {!! Form::hidden('digitalCatalogsId', $text->digitalCatalogsId) !!}
                                {!! Form::button('<i class="fa fa-trash"></i>', ['title'=>'Excluir', 'data-toggle'=>'tooltip', 'class'=>'btn btn-xs btn-danger btn-delete',
                                'data-url'=>route('digitalCatalogsDelete'), 'data-form'=>true, 'data-id-form'=>'textDelete'.$text->digitalCatalogsId]) !!}
                                {!! Form::close() !!}
                            </td>
                        </tr>
                    @endforeach
