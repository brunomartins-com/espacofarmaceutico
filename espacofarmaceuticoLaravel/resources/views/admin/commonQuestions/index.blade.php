        ...
        <div class="block">
            <div class="block-header bg-gray-lighter">
                ...
                <h3 class="block-title">
                    {!! Form::button('<i class="fa fa-plus"></i> Adicionar', ['class'=>'btn btn-primary', 'onclick'=>'window.open(\''.route('commonQuestionsAdd').'\', \'_self\');']) !!}
                    {!! Form::button('<i class="fa fa-list"></i> Ordenar', ['class'=>'btn btn-warning push-10-l', 'onclick'=>'window.open(\''.route('commonQuestionsOrder').'\', \'_self\');']) !!}
                </h3>
            </div>
            <div class="block-content">
                ...
                <table class="table table-bordered table-striped js-dataTable-full">
                    ...
                    <tbody>
                    @foreach($commonQuestions as $text)
                        <tr>
                            <td>{{ $text->sortorder }}</td>
                            <td class="font-w600">{{ $text->question }}</td>
                            <td class="text-center">
                                {!! Form::button('<i class="fa fa-pencil"></i>', ['title'=>'Editar', 'data-toggle'=>'tooltip', 'class'=>'btn btn-xs btn-primary',
                                'onclick'=>'window.open(\''.route('commonQuestionsEdit', $text->commonQuestionsId).'\', \'_self\')']) !!}

                                {!! Form::open([
                                    'id' => 'textDelete'.$text->commonQuestionsId,
                                    'method' => 'delete',
                                    'enctype' => 'multipart/form-data',
                                    'url' => ''
                                    ])
                                !!}
                                {!! Form::hidden('commonQuestionsId', $text->commonQuestionsId) !!}
                                {!! Form::button('<i class="fa fa-trash"></i>', ['title'=>'Excluir', 'data-toggle'=>'tooltip', 'class'=>'btn btn-xs btn-danger btn-delete',
                                'data-url'=>route('commonQuestionsDelete'), 'data-form'=>true, 'data-id-form'=>'textDelete'.$text->commonQuestionsId]) !!}
                                {!! Form::close() !!}
                            </td>
                        </tr>
                    @endforeach
