...
@section('title', 'Ordenar (Perguntas Frequentes) | ')
                ...
                <div class="block-content block-content-full">
                    ...
                    <div class="row js-draggable-items">
                        <div class="col-sm-12 draggable-column">
                            ...
                            !!}
                            {!! Form::hidden('databaseTable', 'commonQuestions') !!}
                            {!! Form::hidden('primaryKey', 'commonQuestionsId') !!}
                            {!! Form::hidden('sortorder', '') !!}
                            @foreach($items as $item)
                                    <!-- Block -->
                            <div class="block draggable-item" title="{{ $item->commonQuestionsId }}">
                                <div class="block-header">
                                    ...
                                    <h3 class="block-title">{{ $item->question }}</h3>
                                </div>
                            </div>
                            <!-- END Block -->
                            @endforeach
                            <div class="form-group">
                                <div class="col-xs-12 push-30-t">
                                    {!! Form::button('Concluído', ['class'=>'btn btn-primary btn-back pull-left', 'data-url'=>route('commonQuestions')]) !!}
                                </div>
                            </div>
                            {!! Form::close() !!}
                        </div>
