...
@section('title', 'Visite Bem - Fotos | ')
        ...
        <div class="block">
            <div class="block-header bg-gray-lighter">
                ...
                <h3 class="block-title">
                    {!! Form::button('<i class="fa fa-plus"></i> Adicionar', ['class'=>'btn btn-primary', 'onclick'=>'window.open(\''.route('visitWellPhotosAdd').'\', \'_self\');']) !!}
                </h3>
            </div>
                    ...
                    @foreach($visitWellPhotos as $photo)
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                'onclick'=>'window.open(\''.route('visitWellPhotosEdit', $photo->visitWellPhotosId).'\', \'_self\')']) !!}

                                {!! Form::open([
                                    'id' => 'textDelete'.$photo->visitWellPhotosId,
                                    'method' => 'delete',
                                    'enctype' => 'multipart/form-data',
                                    'url' => ''
                                    ])
                                !!}
                                {!! Form::hidden('visitWellPhotosId', $photo->visitWellPhotosId) !!}
                                {!! Form::hidden('image', $photo->image) !!}
                                {!! Form::button('<i class="fa fa-trash"></i>', ['title'=>'Excluir', 'data-toggle'=>'tooltip', 'class'=>'btn btn-xs btn-danger btn-delete',
                                'data-url'=>route('visitWellPhotosDelete'), 'data-form'=>true, 'data-id-form'=>'textDelete'.$photo->visitWellPhotosId]) !!}
                                {!! Form::close() !!}
                            </td>
                        </tr>
                    @endforeach
