        ...
        <div class="block">
            <div class="block-header bg-gray-lighter">
                ...
                <h3 class="block-title">
                    {!! Form::button('<i class="fa fa-plus"></i> Adicionar', ['class'=>'btn btn-primary', 'onclick'=>'window.open(\''.route('conceptAdd').'\', \'_self\');']) !!}
                    {!! Form::button('<i class="fa fa-list"></i> Ordenar', ['class'=>'btn btn-warning push-10-l', 'onclick'=>'window.open(\''.route('conceptOrder').'\', \'_self\');']) !!}
                </h3>
            </div>
                    ...
                    @foreach($concept as $text)
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                'onclick'=>'window.open(\''.route('conceptEdit', $text->conceptId).'\', \'_self\')']) !!}

                                {!! Form::open([
                                    'id' => 'textDelete'.$text->conceptId,
                                    'method' => 'delete',
                                    'enctype' => 'multipart/form-data',
                                    'url' => ''
                                    ])
                                !!}
                                {!! Form::hidden('conceptId', $text->conceptId) !!}
                                {!! Form::button('<i class="fa fa-trash"></i>', ['title'=>'Excluir', 'data-toggle'=>'tooltip', 'class'=>'btn btn-xs btn-danger btn-delete',
                                'data-url'=>route('conceptDelete'), 'data-form'=>true, 'data-id-form'=>'textDelete'.$text->conceptId]) !!}
                                {!! Form::close() !!}
                            </td>
                        </tr>
                    @endforeach
